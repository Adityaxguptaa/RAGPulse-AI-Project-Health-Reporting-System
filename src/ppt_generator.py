...
def _text(slide, left, top, w, h, txt, size=12, bold=False,
          color=_WHITE, center=False, wrap=True):
...
def _header_bar(slide, prs, title: str, bg=_DARK):
    ...
    _text(slide, Inches(0.3), Inches(0.1), W - Inches(0.6), Inches(0.55),
...
def _footer_bar(slide, prs, left_txt: str, right_txt: str, bg=_DARK):
    ...
    _text(slide, Inches(0.3), H - Inches(0.42), W * 0.5, Inches(0.38),
          left_txt, size=9, color=_WHITE)
    _text(slide, W * 0.5, H - Inches(0.42), W * 0.5 - Inches(0.3), Inches(0.38),
          right_txt, size=9, color=_WHITE, center=True)
...
def _p_title(prs, summary):
    ...
    _text(s, Inches(1), Inches(1.2), W - Inches(2), Inches(0.6),
          "Monthly Portfolio Health Report", size=34, bold=True, color=_WHITE, center=True)
    _text(s, Inches(1), Inches(2.0), W - Inches(2), Inches(0.45),
          "AI-Powered RAG Project Health Analysis", size=16, color=_GREY, center=True)
    _text(s, Inches(1), int(H*0.56) + Inches(0.05), W - Inches(2), Inches(0.45),
          f"Report Date: {summary.get('report_date', date.today().isoformat())}  |  "
          f"{summary.get('total_projects', 0)} Projects Analysed",
          size=13, bold=True, color=_WHITE, center=True)
    _text(s, Inches(1), H - Inches(0.45), W - Inches(2), Inches(0.38),
...
def _p_overview(prs, summary):
    ...
    sig_c = _RED if "high risk" in signal.lower() else (_AMBER if "pressure" in signal.lower() else _GREEN)

    kpis = [("Total", str(total), _DARK), ("🟢 Green", str(dist.get("Green",0)), _GREEN),
            ("🟡 Amber", str(dist.get("Amber",0)), _AMBER), ("🔴 Red", str(dist.get("Red",0)), _RED),
            ("Avg Score", str(avg), _BLUE)]
    bw, bh, gap, sx, ty = Inches(2.2), Inches(1.1), Inches(0.22), Inches(0.5), Inches(0.85)
    for i, (lbl, val, col) in enumerate(kpis):
        x = sx + i*(bw+gap)
        _box(s, x, ty, bw, bh, fill=col)
        _text(s, x, ty+Inches(0.08), bw, Inches(0.55), val, size=26, bold=True, color=_WHITE, center=True)
        _text(s, x, ty+Inches(0.65), bw, Inches(0.35), lbl, size=10, color=_WHITE, center=True)

    _box(s, Inches(0.5), Inches(2.15), W - Inches(1), Inches(0.5), fill=sig_c)
    _text(s, Inches(0.6), Inches(2.15), W - Inches(1.2), Inches(0.5),
          f"  Portfolio Signal: {signal}", size=13, bold=True, color=_WHITE)

    max_c  = max(dist.get("Green",0), dist.get("Amber",0), dist.get("Red",0), 1)
    bar_h  = Inches(2.4)
    bar_w  = Inches(1.8)
    bar_ty = Inches(2.85)
    for i, (rag, col) in enumerate([("Green",_GREEN),("Amber",_AMBER),("Red",_RED)]):
        cnt = dist.get(rag, 0)
        bh2 = int(bar_h * cnt / max_c) if max_c else 0
        bx  = Inches(2.5 + i*(bar_w + Inches(0.6)))
        if bh2 > 0:
            _box(s, bx, bar_ty + bar_h - bh2, bar_w, bh2, fill=col)
        _text(s, bx, bar_ty+bar_h+Inches(0.06), bar_w, Inches(0.4),
              f"{rag}  {cnt}", size=12, bold=True, color=_DARK, center=True)
...
def _p_scorecard(prs, projects):
    ...
    cw   = [Inches(2.6), Inches(1.1), Inches(0.9), Inches(0.9), Inches(0.9), Inches(0.8), Inches(5.5)]
    row_h= Inches(0.5)
    hdr_y= Inches(0.85)
    x = Inches(0.25)
    for ci, (col, cwidth) in enumerate(zip(cols, cw)):
        _box(s, x, hdr_y, cwidth, row_h, fill=_DARK)
        _text(s, x, hdr_y, cwidth, row_h, col, size=10, bold=True, color=_WHITE, center=True)
        x += cwidth
    ...
    for ri, p in enumerate(projects[:10]):
        ...
        for ci, (val, cwidth) in enumerate(zip(vals, cw)):
            if ci == 1:
                ...
                bw2 = Inches(0.8)
                _box(s, x+(cwidth-bw2)//2, ry+Inches(0.08), bw2, Inches(0.32), fill=_RAG_RGB.get(rag,_GREY))
                _text(s, x+(cwidth-bw2)//2, ry+Inches(0.08), bw2, Inches(0.32),
                      rag, size=8, bold=True, color=_WHITE, center=True)
            else:
                _box(s, x, ry, cwidth, row_h, fill=bg)
                _text(s, x+Inches(0.05), ry, cwidth-Inches(0.05), row_h,
                      val, size=9, color=_DARK, center=(ci>1 and ci<6))
            x += cwidth
...
def _p_risks(prs, projects):
    ...
    for proj, txt, st in all_risks:
        ...
        if key not in seen and txt:
            seen.add(key); unique.append((proj,txt,st))
        if len(unique) >= 10: break

    if not unique:
        _text(s, Inches(0.5), Inches(1.2), W-Inches(1), Inches(1),
              "No significant risks identified.", size=14, color=_DARK)
    else:
        row_h = Inches(0.52)
        for i, (proj, txt, status) in enumerate(unique):
            y   = Inches(0.88) + i*row_h
            rgb = _RAG_RGB.get(status, _GREY)
            bg  = (253,237,236) if status=="Red" else (254,249,231) if status=="Amber" else (213,245,227)
            _box(s, Inches(0.25), y, W-Inches(0.5), row_h-Inches(0.04), fill=bg)
            _box(s, Inches(0.25), y, Inches(0.1),   row_h-Inches(0.04), fill=rgb)
            _text(s, Inches(0.45), y, Inches(2.2), row_h-Inches(0.04),
                  f"[{proj[:22]}]", size=9, bold=True, color=_DARK)
            _text(s, Inches(2.65), y, W-Inches(3), row_h-Inches(0.04),
                  txt[:130], size=9, color=_DARK)
...
def _p_recommendations(prs, projects):
    ...
    for p in projects:
        for rec in (p.recommendations or [])[:2] + (p.next_week_priorities or [])[:1]:
            if rec: entries.append((p.name, rec))
        if len(entries) >= 12: break

    if not entries:
        _text(s, Inches(0.5), Inches(1.2), W-Inches(1), Inches(1),
              "Enable Gemini AI to generate recommendations.", size=14, color=_DARK)
    else:
        ...
        for ci, col in enumerate([left_col, right_col]):
            ...
            for ri, (proj, rec) in enumerate(col):
                ...
                _text(s, x+Inches(0.18), y, Inches(6), Inches(0.22),
                      f"[{proj[:24]}]", size=8, bold=True, color=_BLUE)
                _text(s, x+Inches(0.18), y+Inches(0.22), Inches(6), Inches(0.26),
                      rec[:110], size=9, color=_DARK)
...
def _p_methodology(prs):
    ...
    for hdr, cwidth in zip(headers, cw):
        _box(s, x, hdr_y, cwidth, row_h, fill=_DARK)
        _text(s, x, hdr_y, cwidth, row_h, hdr, size=10, bold=True, color=_WHITE, center=True)
        x += cwidth
    ...
    for ri, (dim, wt, criteria) in enumerate(dims):
        ...
        for val, cwidth in zip([dim, wt, criteria], cw):
            ...
            _text(s, x+Inches(0.08), ry, cwidth-Inches(0.1), row_h-Inches(0.04),
                  val, size=10, color=_DARK, center=(cwidth < Inches(1.5)))
            x += cwidth
    ...
    for i, (lbl, desc, col) in enumerate([
        ...
        _text(s, tx, ty, Inches(3.2), Inches(0.28), lbl, size=12, bold=True, color=_WHITE, center=True)
        _text(s, tx, ty+Inches(0.28), Inches(3.2), Inches(0.25), desc, size=9, color=_WHITE, center=True)
...
def _p_closing(prs, summary, projects):
    ...
    _text(s, Inches(0.5), Inches(0.1), W-Inches(1), Inches(0.65),
          "Executive Summary & Closing", size=24, bold=True, color=_WHITE)
    signal  = summary.get("health_signal","")
    sig_c   = _RED if "high risk" in signal.lower() else (_AMBER if "pressure" in signal.lower() else _GREEN)
    _box(s, Inches(0.5), Inches(1.0), W-Inches(1), Inches(0.55), fill=sig_c)
    _text(s, Inches(0.6), Inches(1.0), W-Inches(1.2), Inches(0.55),
          f"  {signal}", size=13, bold=True, color=_WHITE)
    y = Inches(1.75)
    for p in projects[:7]:
        rag = p.rag_status or "Unknown"
        sym = {"Green":"🟢","Amber":"🟡","Red":"🔴"}.get(rag,"⚪")
        _text(s, Inches(0.5), y, W-Inches(1), Inches(0.48),
              f"{sym}  {p.name}:  {(p.executive_summary or 'No summary')[:130]}",
              size=10, color=_WHITE)
        y += Inches(0.52)
    _box(s, 0, H-Inches(0.5), W, Inches(0.5), fill=_BLUE)
    _text(s, Inches(0.5), H-Inches(0.48), W-Inches(1), Inches(0.4),
...
def _proj_title(prs, project):
    ...
    _text(s, W//2-Inches(1.5), Inches(2.8), Inches(3), Inches(0.55),
          rag, size=16, bold=True, color=_WHITE, center=True)
    _text(s, Inches(1), Inches(3.55), W-Inches(2), Inches(0.4),
...
def _proj_dashboard(prs, project):
    ...
    kpis  = [
        ...
        ("Tasks Done",  f"{m.get('completed_tasks',0)}/{m.get('total_tasks',0)}", _GREEN),
    ...
    bw, bh, gap, sx, ty = Inches(1.9), Inches(0.95), Inches(0.18), Inches(0.35), Inches(0.85)
    for i, (lbl, val, c) in enumerate(kpis):
        x = sx + i*(bw+gap)
        _box(s, x, ty, bw, bh, fill=c)
        _text(s, x, ty+Inches(0.1), bw, Inches(0.55), val, size=22, bold=True, color=_WHITE, center=True)
        _text(s, x, ty+Inches(0.65), bw, Inches(0.25), lbl, size=9, color=_WHITE, center=True)

    if project.executive_summary:
        _box(s, Inches(0.3), Inches(2.0), W-Inches(0.6), Inches(0.85), fill=(234,244,251))
        _text(s, Inches(0.45), Inches(2.0), W-Inches(0.9), Inches(0.85),
              (project.executive_summary or "")[:220], size=10, color=_DARK)
    ...
    for dim_name, data in dims:
        ...
        _text(s, Inches(0.3), dy, Inches(2.5), Inches(0.35),
              dim_name.replace("_"," ").title(), size=9, bold=True, color=_DARK)
        _box(s, Inches(2.9), dy+Inches(0.05), bar_max_w, Inches(0.26), fill=(220,220,220))
        if filled > 0:
            _box(s, Inches(2.9), dy+Inches(0.05), filled, Inches(0.26), fill=d_col)
        _text(s, Inches(2.95), dy+Inches(0.05), Inches(0.8), Inches(0.26),
              f"{score:.2f}", size=8, bold=True, color=_WHITE)
        _text(s, Inches(2.9)+bar_max_w+Inches(0.1), dy, Inches(1.0), Inches(0.35),
              f"{wt:.0%}", size=8, color=_GREY)
        _text(s, Inches(2.9)+bar_max_w+Inches(1.2), dy, Inches(2.8), Inches(0.35),
              rat, size=8, color=_GREY)
        dy += Inches(0.42)
...
def _proj_progress(prs, project):
    ...
    if overdue:
        ...
        _text(s, Inches(0.35), oy, W-Inches(0.7), Inches(0.35),
              f"  ⚠  {len(overdue)} Overdue Milestone(s)", size=11, bold=True, color=_WHITE)
        oy += Inches(0.38)
        for ms in overdue[:3]:
            _box(s, Inches(0.25), oy, W-Inches(0.5), Inches(0.38), fill=(253,237,236))
            _text(s, Inches(0.35), oy, W-Inches(0.7), Inches(0.38),
    ...
    cw     = [Inches(4.2), Inches(1.3), Inches(0.9), Inches(0.7), Inches(0.7), Inches(0.85), Inches(1.0)]
    row_h  = Inches(0.42)
    x = Inches(0.25)
    for col, cwidth in zip(cols, cw):
        _box(s, x, oy, cwidth, row_h, fill=_DARK)
        _text(s, x, oy, cwidth, row_h, col, size=9, bold=True, color=_WHITE, center=True)
        x += cwidth
    ...
    for ri, t in enumerate(all_tasks[:max_rows]):
        ...
        for val, cwidth in zip(vals, cw):
            _box(s, x, ry, cwidth, row_h, fill=bg)
            _text(s, x+Inches(0.06), ry, cwidth-Inches(0.06), row_h,
                  val, size=8, color=_DARK, center=(cwidth < Inches(1.2)))
            x += cwidth
...
def _proj_risks_slide(prs, project):
    ...
    if all_risks:
        ...
        cw   = [Inches(0.5), Inches(5.8), Inches(1.1), Inches(1.2), Inches(2.2), Inches(1.5)]
        x    = Inches(0.25)
        for col, cwidth in zip(cols, cw):
        ...
        for ri, r in enumerate(all_risks):
            ry  = oy + row_h + ri*row_h
            bg  = (253,237,236) if (r.severity or "").lower() in {"high","critical"} else \
                  (254,249,231) if (r.severity or "").lower()=="medium" else (213,245,227)
            vals= [str(ri+1), (r.description or "")[:70], r.severity or "",
                   r.probability or "", (r.owner or "")[:25], r.status or ""]
            x   = Inches(0.25)
            for val, cwidth in zip(vals, cw):
                _box(s, x, ry, cwidth, row_h, fill=bg)
                _text(s, x+Inches(0.05), ry, cwidth-Inches(0.05), row_h,
                      val, size=8, color=_DARK, center=(cwidth < Inches(1.5)))
                x += cwidth
    elif id_risks:
        for i, r in enumerate(id_risks[:10]):
            ry = oy + i*Inches(0.5)
            _box(s, Inches(0.25), ry, W-Inches(0.5), Inches(0.44), fill=(253,237,236))
            _box(s, Inches(0.25), ry, Inches(0.1),   Inches(0.44), fill=_RED)
            _text(s, Inches(0.42), ry, W-Inches(0.7), Inches(0.44), r[:130], size=10, color=_DARK)
    else:
        _text(s, Inches(0.5), Inches(1.5), W-Inches(1), Inches(1),
              "No risks identified.", size=14, color=_DARK)
...
def _proj_recs_slide(prs, project):
    ...
    _text(s, Inches(0.3), Inches(0.85), col_w, Inches(0.35),
          "✅  Recommendations", size=12, bold=True, color=_DARK)
    _text(s, Inches(0.45)+col_w, Inches(0.85), col_w, Inches(0.35),
          "📅  Next Week Priorities", size=12, bold=True, color=_DARK)
    ...
    for i, rec in enumerate(recs[:8]):
        ...
        _text(s, Inches(0.48), y, col_w-Inches(0.25), Inches(0.48),
              rec[:120], size=9, color=_DARK)
    ...
    for i, p in enumerate(prios[:8]):
        ...
        _text(s, Inches(0.63)+col_w, y, col_w-Inches(0.25), Inches(0.48),
              p[:120], size=9, color=_DARK)

    if missing:
        by = H - Inches(1.6)
        _box(s, Inches(0.3), by, W-Inches(0.6), Inches(0.32), fill=(231,76,60))
        _text(s, Inches(0.4), by, W-Inches(0.8), Inches(0.32),
              "  ⚠  Missing / Low Quality Data", size=10, bold=True, color=_WHITE)
        for i, d in enumerate(missing[:2]):
            _text(s, Inches(0.4), by+Inches(0.34)+i*Inches(0.3), W-Inches(0.8), Inches(0.28),
                  f"• {d[:130]}", size=9, color=_DARK)
